---
title: "**Omics Informatics Table**"
output: html_document
---

### **Overview**

The following table contains a list of "omics" informatics platforms funded by ITCR grants. "Omics" platforms are those which work with several genomic or proteomic data types, such as sequencing, imaging, text and microarray data. These platforms allow access to interactively exploring and analyzing large-scale cancer data, create and share reproducible workspaces and results. The goal of these platforms include facilitating scientific collaboration and empowering the research community for genomic research. Note that, all resource summaries were taken from their respective sources.
...
generalData <- read_csv(file = here::here("data/ictr_table_omics.csv"))
generalData
...
LinkUpdater <- cSplit_e(generalData %>% select(Name, `Data Provided`), "Data Provided", ",", type = "character", fill = 0, drop = T) %>% 
  set_names(str_replace(string = names(.), pattern = "Data Provided_", replacement = "")) %>% 
  pivot_longer(cols = -Name, names_to = "identifier", values_to = "indicator") %>% 
  filter(indicator == 1) %>%
